...
class AgentMessage:
    ...
    def from_openai_message(cls, payload: JSONDict) -> 'AgentMessage':
        """从 OpenAI 格式的字典反序列化为 AgentMessage 实例。"""
...
def _render_user_context_reminder(user_context: dict[str, str]) -> str:
    """将用户上下文渲染为 system-reminder 格式的提示文本。"""
    body = '\n'.join(
        f'# {key}\n{value}'
